...
// Check and add rating fields to courses table
try {
    // Define rating columns to check and add
    $ratingColumns = [
    ...
    // Add any missing rating columns
    foreach ($ratingColumns as $column => $type) {
        if (!in_array($column, $existingColumns)) {
            echo "Adding '$column' field to the courses table...\n";
            $db->exec("ALTER TABLE courses ADD COLUMN $column $type");
            echo "Added '$column' field to the courses table.\n";
        } else {
            echo "The '$column' field already exists in the courses table.\n";
        }
    }
} catch (Exception $e) {
    echo "Error checking or adding rating fields: " . $e->getMessage() . "\n";
}
...
try {
    ...
    foreach ($data['courses'] as $course) {
        ...
        if ($existingCourse) {
            ...
            $insertStmt = $db->prepare('
                INSERT INTO courses (name, course_code, description, professor_id, semester, avg_content_quality, avg_difficulty, overall_rating, review_count) 
                VALUES (:name, :course_code, :description, :professor_id, :semester, 0, 0, 0, 0)
            ');
    ...
    try {
        // Check if professor rating columns exist and add them if missing
        $profResult = $db->query("PRAGMA table_info(professors)");
        $hasRatingColumns = false;
        $existingProfColumns = [];
        
        while ($row = $profResult->fetchArray(SQLITE3_ASSOC)) {
            $existingProfColumns[] = $row['name'];
        }
        
        // Check if we need to add rating columns to professors table
        if (!in_array('avg_content_quality', $existingProfColumns)) {
            $db->exec("ALTER TABLE professors ADD COLUMN avg_content_quality REAL DEFAULT 0");
            echo "Added avg_content_quality to professors table.\n";
        }
        
        if (!in_array('avg_difficulty', $existingProfColumns)) {
            $db->exec("ALTER TABLE professors ADD COLUMN avg_difficulty REAL DEFAULT 0");
            echo "Added avg_difficulty to professors table.\n";
        }
        
        if (!in_array('overall_rating', $existingProfColumns)) {
            $db->exec("ALTER TABLE professors ADD COLUMN overall_rating REAL DEFAULT 0");
            echo "Added overall_rating to professors table.\n";
        }
        
        if (!in_array('review_count', $existingProfColumns)) {
            $db->exec("ALTER TABLE professors ADD COLUMN review_count INTEGER DEFAULT 0");
            echo "Added review_count to professors table.\n";
        }
        
        $hasRatingColumns = true;
    } catch (Exception $e) {
        echo "Note: Could not add rating columns to professors table. " . $e->getMessage() . "\n";
    }
