  ...
  while (hasNextPage) {
    ...
    for (let i = 0; i < syllabusLinks.length; i++) {
      ...
      if (englishLink) {
        ...
        const course = {
          course_id: japaneseDetails.course_id || englishDetails.course_id, // Still include for reference
          year: year,
          semester: japaneseDetails.semester.toLowerCase().includes('春') ? 'spring' : 'fall', // Extract semester
          translations: {
            ja: {
